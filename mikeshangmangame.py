...
def get_new_word() -> str:
    with open("usa.txt") as file:
        ...
        dictionary = file_raw.split("\n")
        length_of_file = len(dictionary)
        line_selection = random.randint(0, length_of_file-1)
        word_selection = dictionary[line_selection]
...
def gameplay(word):
    ...
    selected_word_for_game: str = get_new_word()
    ...
    while not game_won and number_of_lives > 0:
        ...
        # if user_guess is in the word, guessed_letters list is updated
            ...
            print(current_progress)
            continue
        if "-" not in gen_word_output(answer, guesses):
            game_won = True
        
    # Game won or lost
    if game_won:
        print("Good job, you won! The word was " + word)
        
    else:
        print("You're out of lives! You're DEAD! The correct word was " + word)
